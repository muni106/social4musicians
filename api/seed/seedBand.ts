...
import { createAlbumByArtist, createAlbumByBand, createBand, createBandMember, createSongByArtist, createSongByBand } from "../queries/dbCreate";
import { getAllArtists, getAllBands, getAllGenres, getAllSongs, getArtistSongs, getBandSongs } from "../queries/dbRead";
...
export async function seedSongs(prisma: PrismaClient) {
    ...
    for(let i = 0; i < bands.length; i++) {
        await createSongByBand(prisma, 3, "default song", 130, bands[i].bandname, new Date() );
    }
    for(let i = 0; i < bands.length; i++) {
        await createSongByBand(prisma, 3, "default song 2", 130, bands[i].bandname, new Date() );
    }

    for(let i = 0; i < users.length; i++) {
        await createSongByArtist(prisma, 2, "default solo song", 60, users[i].nickname, new Date());
    }
    for(let i = 0; i < users.length; i++) {
        await createSongByArtist(prisma, 2, "default solo song 2", 60, users[i].nickname, new Date());
    }
}
...
export async function seedAlbums(prisma: PrismaClient) {
    ...
    for (let i = 0; i < allArtists.length; i++) {
        let currAlbum = await createAlbumByArtist(prisma, "default album", new Date(), allArtists[i].nickname ); 
    ...
    for (let i = 0; i < allBands.length; i++) {
        let currAlbum = await createAlbumByBand(prisma, "default solo album", new Date(), allBands[i].bandname );
