...
import { updateHashtagObserved } from "../queries/dbUpdates";
...
export async function seedDiscussions(prisma: PrismaClient) {
  try {
    await createDiscussion(
      ...
      3.8,
      "guitarist1",
      "Debating the merits of rock and pop music."
    );
    await createDiscussion(
      prisma,
      new Date(),
      ...
      4.7,
      "drummer1",
      "Discussing the greatest classical composers."
    );
    await createDiscussion(
      prisma,
      new Date(),
      ...
      4.1,
      "saxophonist1",
      "Sharing experiences from electronic music festivals."
    );
    await createDiscussion(
      prisma,
      new Date(),
      ...
      4.6,
      "violist1",
      "Analyzing the lyrics of popular hip-hop songs."
    );
    await createDiscussion(
      prisma,
      new Date(),
      ...
      4.5,
      "cellist1",
      "Celebrating iconic country music artists."
    );
    await createDiscussion(
      prisma,
      new Date(),
      ...
      4.3,
      "sitarplayer1",
      "Exploring the reggae music culture."
    );
    await createDiscussion(
      prisma,
      new Date(),
      ...
      4.4,
      "pianist2",
      "Diving into the diverse world of metal subgenres."
    );
    await createDiscussion(
      prisma,
      new Date(),
      ...
      4.2,
      "guitarist1",
      "Debating the perfect pizza toppings."
    );
    await createDiscussion(
      prisma,
      new Date(),
      "Cat Memes Appreciation",
      4.7,
      "singer2",
      "Sharing and discussing funny cat memes."
    );
    await createDiscussion(
      prisma,
      new Date(),
      ...
      4.5,
      "bassist2",
      "Fantasy showdown between superheroes."
    );
    await createDiscussion(
      prisma,
      new Date(),
      ...
      4.3,
      "keyboardist2",
      "Sharing favorite ice cream flavors and combinations."
    );
    await createDiscussion(
      prisma,
      new Date(),
      ...
      4.8,
      "trumpetplayer2",
      "Sharing and laughing at cringe-worthy dad jokes."
    );
    await createDiscussion(
      prisma,
      new Date(),
      ...
      4.1,
      "trumpetplayer2",
      "Discussing wild theories about space aliens."
    );
    await createDiscussion(
      prisma,
      new Date(),
      "Best Pizza Slice Folding Technique",
      4.6,
      "trumpetplayer2",
      "Debating the art of folding pizza slices for maximum enjoyment."
    );
    await createDiscussion(
      prisma,
      new Date(),
      ...
      4.2,
      "bassist2",
      "Discussing memorable movie soundtracks."
    );
    await createDiscussion(
      prisma,
      new Date(),
      ...
      4.0,
      "pianist2",
      "Exploring the indie rock music scene."
    );
    await createDiscussion(
      prisma,
      new Date(),
      ...
      4.5,
      "saxophonist1",
      "Sharing tips and tricks for electronic music production."
    );
    await createDiscussion(
      prisma,
      new Date(),
      ...
      4.3,
      "flutist2",
      "Appreciating the diversity of Latin music."
    );
    await createDiscussion(
      prisma,
      new Date(),
      ...
      4.6,
      "drummer2",
      "Celebrating legendary blues guitarists."
    );
    await createDiscussion(
      prisma,
      new Date(),
      ...
      4.4,
      "cellist1",
      "Discussing popular reggaeton hits."
    );
    await createDiscussion(
      prisma,
      new Date(),
      ...
      4.1,
      "violist1",
      "Exploring the world of progressive metal music."
    );
    await createDiscussion(
      prisma,
      new Date(),
      "Best Air Guitar Performances",
      4.0,
      "flutist1",
      "Ranking the most epic air guitar performances."
    );
    await createDiscussion(
      prisma,
      new Date(),
      "Silliest Song Lyrics Ever",
      4.2,
      "percussionist1",
      "Sharing and laughing at silly song lyrics."
    );
    await createDiscussion(
      prisma,
      new Date(),
      ...
      3.8,
      "cellist1",
      "Discussing the most cringe-worthy album cover designs."
    );
    await createDiscussion(
      prisma,
      new Date(),
      "Unbelievable Band Names",
      4.1,
      "guitarist2",
      "Sharing bizarre and hilarious band names."
    );
    await createDiscussion(
      prisma,
      new Date(),
      ...
      4.3,
      "guitarist1",
      "Enjoying and discussing musical parodies and spoofs."
    );
    await createDiscussion(
      prisma,
      new Date(),
      "Most Memorable Karaoke Fails",
      3.9,
      "drummer1",
      "Recalling unforgettable karaoke mishaps and performances."
    );
    await createDiscussion(
      prisma,
      new Date(),
      "Musical Misheard Lyrics",
      4.4,
      "guitarist1",
      "Sharing funny misunderstandings of song lyrics."
    );
  } catch (error) {
    console.log("error on seedDiscussions: ", error);
  }
}
...
export async function seedComments(prisma: PrismaClient) {
  try {
    ...
    console.log("errors found on seedComments: ", error);
  }
}
...
export async function seedReactions(prisma: PrismaClient) {
  try {
    ...
    console.log("error on seedReactions: ", error);
  }
}
...
export async function seedHashtags(prisma: PrismaClient) {
  try {
    ...
    console.log("error on seedHashtags: ", error);
  }
}

export async function seedPostReferences(prisma: PrismaClient) {
  try {
    const disc = await getAllDiscussions(prisma);
    const hashtags = await getAllHashtags(prisma);
    disc.forEach((x) => {
      let currentHT = [...hashtags].sort(() => 0.5 - Math.random());
      for (let index = 0; index < 3; index++) {
        addHashtagToPost(prisma, x.discussionid, currentHT[index].hashtagname);
      }
    });
  } catch (error) {
    console.log("error on seedPostReferences: ", error);
  }
}

export async function seedHashtagFollows(prisma: PrismaClient) {
  try {
    ...
    artists.forEach((x) => {
      let currentHT = [...hashtags].sort(() => 0.5 - Math.random());
      for (let index = 0; index < 3; index++) {
        updateHashtagObserved(prisma, x.nickname, currentHT[index].hashtagname);
      }
    });
  } catch (error) {
    console.log("error on seedHashtagFollows: ", error);
  }
}
